...
class Juego:
    ...
    def cargar_nivel(self):
        ...
        try:
            with open(self.niveles_files[self.nivel_actual], 'r', encoding="utf-8") as file:
                text = file.read()
    ...
    def crear_ui(self):
        ...
        # Barra de vida
        self.barra_vida_canvas = tk.Canvas(self.ventana, width=200, height=20, bg="grey")
        self.barra_vida_canvas.pack(pady=10)
        self.actualizar_barra_vida()
        
        # Marcador de puntos
        self.marcador_puntos = tk.Label(self.ventana, text=f"Puntos: {self.jugador.puntos}", font=("Arial", 12))
    ...
    def revisar_casilla(self, x, y):
        ...
        if isinstance(contenido, Barco):
            ...
            messagebox.showinfo("Tesoro encontrado", f"¡Encontraste {contenido.nombre} de {contenido.puntos} puntos!")
            ...
            messagebox.showwarning("Obstáculo", f"Encontraste una {contenido.nombre} que te quita {contenido.daño} puntos de vida.")
            self.actualizar_barra_vida()
        else:
        ...
        if self.aciertos >= self.nivel.numero_barcos:
            messagebox.showinfo("Nivel completado", "¡Has encontrado todos los tesoros de este nivel!")
            self.siguiente_nivel()
            
        # Verificar si el jugador sigue vivo
        if self.jugador.vida <= 0:
            messagebox.showinfo("Juego terminado", "Has perdido toda tu vida. ¡Juego terminado!")
            self.ventana.destroy()
    ...
    def actualizar_barra_vida(self):
        ...
        self.barra_vida_canvas.create_rectangle(0, 0, vida_actual, 20, fill="green", tags="vida")
